...
def main():
    ...
    model = parser.get_one_option("method", "Kmeans")
    ...
    Centroide_and_cluster(method, nb_clusters, data_selection)

    if parser.has_option("visual"):
        Visualisation(method, data_selection, data_complete)

    nb_clusters_to_test = int(parser.get_one_option("nb_clusters_to_test", "10"))

    if parser.has_option("curve"):
        Courbe(nb_clusters_to_test, data_selection)
